...
try:
    ...
    import Quandl as qd
import fredapi
import pystocklib.utilities as ut
...
future_yrs_long = 8
future_yrs_short = 1
date_plotend = pd.Timestamp.today()
#########################################

MonthNameToNumber = dict(JAN=1, FEB=2, MAR=3, APR=4, MAY=5, JUN=6, JUL=7, AUG=8, SEP=9, OCT=10, NOV=11, DEC=12)

date_plotstart = date_plotend - pd.Timedelta(days=int(macro_yrs_ultralong*365.25))
date_future_plotstart_long = date_plotend - pd.Timedelta(days=int(future_yrs_long*365.25))
date_future_plotstart_short = date_plotend - pd.Timedelta(days=int(future_yrs_short*365.25))

legend_fontsize = 10
maxattempts = 20
dpi = 109
figsize = (2560/dpi, 1440/dpi)
...
cfgfn = '../config/config.xml'
qd_authtoken = ut.GetQuandlConfigFromXML(fn=cfgfn)['authtoken']
fred_apikey = ut.GetFREDConfigFromXML(fn=cfgfn)['apikey']
print('qd_authtoken: ', qd_authtoken)
print('fred_apikey: ', fred_apikey)

print('Downloading data from Quandl, FRED, and S&P...\n')

flag_downloaded = False
nattempts = 0
while (not flag_downloaded) and nattempts < maxattempts:
    nattempts += 1
    try:
        ...
        # S&P500 index
        print('\tS&P500')
        SP500 = qd.get('YAHOO/INDEX_GSPC', trim_start=date_plotstart, trim_end=date_plotend, authtoken=qd_authtoken)

        # Gold price
        print('\tGold price')
        gold = qd.get('BUNDESBANK/BBK01_WT5511', trim_start=date_plotstart, trim_end=date_plotend, authtoken=qd_authtoken)

        # S&P500 / Gold
        SP500_gold = SP500['Close']/gold.iloc[:, 0]
...
if flag_downloaded:
    ...
    ax.plot(SP500.index, SP500['Close'], 'b-', label='S&P500')
    ax.set_xlim([date_plotstart, date_plotend])
    ax.legend(prop={'size': legend_fontsize}, loc='upper left')
    ax2 = ax.twinx()
    ax2.plot(gold.index, gold.iloc[:, 0], 'r-', label='Gold')
    ax2.set_ylabel('$/OZ')
    ax2.set_xlim([date_plotstart, date_plotend])
    ax2.legend(prop={'size': legend_fontsize}, loc='lower right')
    plt.grid(True)
    plt.title('Stock and Gold as of %s' % today)

    nplot += 1
    ax = plt.subplot(nrows, ncols, nplot)
    ax.plot(SP500_gold.index, np.array(SP500_gold), 'b-')
    ax.set_xlim([date_plotstart, date_plotend])
    # ax.legend(prop={'size': legend_fontsize}, loc='upper left')
    plt.grid(True)
    plt.title('S&P500 / Gold as of %s' % today)
    ...
    for n, city in enumerate(cities_of_interest):
        ax.plot(caseshiller[city].index, np.array(caseshiller[city].ix[:, 1]), '-', color=colors[n],
                linewidth=3 if city in ['National', 'Chicago', 'SanFrancisco'] else 1, label=city)
    ax.set_xlim([date_plotstart, date_plotend])
    # ax.set_yscale('log')
    ax.yaxis.set_major_formatter(fmt)
    ax.legend(prop={'size': legend_fontsize}, loc='upper left')
    ...
    for comdty in futures_underlying:
        ...
        if nplot == int(np.round(ncols/2+0.01)):
            ax.set_title('Futures - Long Term ({:d} year{:s}) as of {:s}'.format(future_yrs_long,
                                                                      's' if future_yrs_long > 1 else '', today))
    ...
    for comdty in futures_underlying:
        ...
        plt.grid(True)
        if nplot == int(np.round(ncols/2+0.01)):
            ax.set_title('Futures - Short Term ({:d} year{:s}) as of {:s}'.format(future_yrs_short,
                                                                       's' if future_yrs_short > 1 else '', today))
